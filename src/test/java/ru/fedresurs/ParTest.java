...
public class ParTest {
    ...
    void commonComplexSearchTest(String testData, String expectedResult) {
        ...
        $(byName("searchString")).setValue(testData);
        $(".btn").click();
        $$(".td_name").get(1).shouldHave(text(expectedResult));
    }

    static Stream<Arguments> dataProviderForSelenideSiteMenuTest() {
        return Stream.of(Arguments.of("6661000096", List.of("Наименование", "ОАО \"УРАЛЭМ\" 620050, ОБЛАСТЬ СВЕРДЛОВСКАЯ, Г. ЕКАТЕРИНБУРГ, УЛ. МАНЕВРОВАЯ, Д.43")), Arguments.of("1027739244741", List.of("Наименование", "ООО \"ИНТЕРНЕТ РЕШЕНИЯ\" 123112, ГОРОД МОСКВА, НАБ. ПРЕСНЕНСКАЯ, Д. 10, ПОМЕЩ. I ЭТ 41 КОМН 6")));
    }
    ...
    void selenideSiteMenuTest(String testData, List<String> expectedResult) {
        ...
        $$(".td_name").shouldHave(CollectionCondition.texts(expectedResult));
